...
func CreateStashers(stash, parent, indx, sfx string, hash, zipp, asn bool, report int, inp <-chan XMLRecord) <-chan string {
	...
	// mutex to protect access to wasDeleted map
	var vlock sync.Mutex

	// map to track inverted index files that were deleted
	...
	stashRecord := func(str, id string, index int) string {
		...
		// calculate .inv.gz file name by dropping last two digits from padded .xml.gz name
		nvid := id
		...
		// protect section with mutex
		vlock.Lock()

		// may be multiple requests to delete the same cached inverted index file
		if !wasDeleted[nvid] {

			// if not already deleted in this session, remove file
			fpath = path.Join(dpath, nvid+".inv.gz")
			os.Remove(fpath)

			// and mark as being deleted
			wasDeleted[nvid] = true

			// will later need to repopulate from new records in directory
		}
...
func CreateCacheStreamers(stash, sfx string, inp <-chan XMLRecord) <-chan XMLRecord {
