...
func evaluateQuery(base, phrase string, clauses []string, noStdout bool) (int, []int32) {
...
func setFieldQualifiers(clauses []string, rlxd bool) []string {
	...
	for _, str := range clauses {
		...
		if strings.HasSuffix(str, " [YEAR]") {
			...
			// load journal table within mutex
			jtAbbrev.lock.Lock()
			if !jtAbbrev.isLoaded {
				jtAbbrev.loadAliasTable(false, true)
			}
			jtAbbrev.lock.Unlock()

			// check journal alias table
			if jtAbbrev.isLoaded {
				alias, ok := jtAbbrev.table[str]
				if ok {
...
func ProcessSearch(base, phrase string, xact, titl, rlxd, deStop bool) int {
	...
	count, _ := evaluateQuery(base, phrase, clauses, false)

	return count
}

// ProcessQuery evaluates query, returns list of PMIDs in array
func ProcessQuery(base, phrase string, xact, titl, rlxd, deStop bool) []int32 {
	...
	_, arry := evaluateQuery(base, phrase, clauses, true)
...
func init() {
	...
	jtAbbrev.table = make(map[string]string)
	...
	if eerr == nil {
		...
		jtAbbrev.fpath = filepath.Join(exPath, "help", "journal-aliases.txt")
	}
