...
func createExternalArchive(stash string, args []string) <-chan string {
	...
	stsq := eutils.CreateStashers(stash, "IdxDocument", "IdxDocument/IdxUid", ".e2x", false, true, false, 50000, mrgr)
...
func main() {
	...
	// -e2invert path to local archive
	archivePath := ""

	// -e2join path to inverted file working directory
	invertPath := ""

	// -e2delete path to local archive (undocumented)
	deletePath := ""

	// -e2clear path to local archive (undocumented)
	clearPath := ""
	...
	for {
		...
		// path to local archive for incremental updating of cached inverted index components
		case "-e2invert":
			archivePath = eutils.GetStringArg(args, "Path to local archive")
			args = args[1:]
			// should be followed by -transform meshtree.txt -e2index

		// path to local archive for deletion of cached inverted index components
		case "-e2delete":
			deletePath = eutils.GetStringArg(args, "Path to local archive")
			args = args[1:]
			// add dummy argument to get past insufficient command-line arguments error
			args = append(args, "-sort")

		// path to local archive for deletion of all cached files in archive
		case "-e2clear":
			// removal of entire archive commented out - uncomment and recompile if needed
			// clearPath = eutils.GetStringArg(args, "Path to local archive")
			args = args[1:]
			// add dummy argument to get past insufficient command-line arguments error
			args = append(args, "-sort")
		...
		case "-e2join":
			archivePath = eutils.GetStringArg(args, "Path to local archive")
			args = args[1:]
			invertPath = eutils.GetStringArg(args, "Path to local invert")
			args = args[1:]
			// add dummy argument to get past insufficient command-line arguments error
			args = append(args, "-sort")
	...
	*/
	...
	// -e2delete DELETION OF LOCAL PUBMED INVERTED INDEX FILES (undocumented)

	if deletePath != "" {

		dinq := eutils.DeleteInvertCache(archivePath)
		if dinq == nil {
			fmt.Fprintf(os.Stderr, "\nERROR: Unable to create deleter channel\n")
			os.Exit(1)
		}

		// drain channel for names of deleted inverted index files
		for range dinq {
			recordCount++
			runtime.Gosched()
		}
	...
	// -e2clear DELETION OF LOCAL PUBMED ARCHIVE AND INVERTED INDEX FILES (undocumented)

	if clearPath != "" {

		dinq := eutils.DeleteArchiveCache(archivePath)
		if dinq == nil {
			fmt.Fprintf(os.Stderr, "\nERROR: Unable to create deleter channel\n")
			os.Exit(1)
		}

		// drain channel for names of deleted archive and inverted index files
		for range dinq {
			recordCount++
			runtime.Gosched()
		}
	...
	// -e2join JOINING OF LOCAL PUBMED INVERTED INDEX FILES

	// create with:
	//   rchive -e2join "${EDIRECT_PUBMED_MASTER}/Archive/" "${EDIRECT_PUBMED_MASTER}/Invert/"

	if archivePath != "" && invertPath != "" {

		jiaq := eutils.JoinInverts(archivePath, invertPath)
		if jiaq == nil {
			fmt.Fprintf(os.Stderr, "\nERROR: Unable to create inverted file joiner channel\n")
			os.Exit(1)
		}

		// drain channel for names of folder-specific inverted index files to be joined
		for itm := range jiaq {
			recordCount++
			runtime.Gosched()
	...
	// -e2invert FOLLOWED BY -transform meshtree.txt AND -e2index

	if len(args) > 0 && args[0] == "-e2index" && archivePath != "" {
		...
		res := eutils.ProcessE2Index(tform, isPipe || usingFile, xtras)
		...
		cmds := eutils.ParseArguments(args, "PubmedArticle")
		if cmds == nil {
			fmt.Fprintf(os.Stderr, "\nERROR: Problem parsing -e2index arguments after -e2invert\n")
			os.Exit(1)
		}
		...
		callConsumers := func(inp <-chan eutils.XMLRecord) <-chan eutils.XMLRecord {
			...
			return eutils.CreateConsumers(cmds, "", "<IdxDocument>", "</IdxDocument>", transform, nil, inp)
		}

		e2iq := eutils.IndexAndInvert(archivePath, callConsumers)
		if e2iq == nil {
			fmt.Fprintf(os.Stderr, "\nERROR: Unable to create indexer inverter channel\n")
			os.Exit(1)
		}
	...
	if len(args) > 0 && args[0] == "-e2index" {
		...
		if !isPipe && !usingFile {
			// no piped input and not -e2invert command, so write output instructions
			fmt.Printf("rchive")
			if tform != "" {
		...
		xmlq := eutils.CreateXMLProducer("PubmedArticle", "", false, rdr)

		// launch consumer goroutines to parse and explore partitioned XML objects
		tblq := eutils.CreateConsumers(cmds, "PubmedArticle", hd, tl, transform, nil, xmlq)
	...
	if base != "" && ttls != "" {
		...
		dpath := path.Join(base, field, ttls)
	...
	if trei {
		...
		for scanr.Scan() {
			...
			if invt {
				// for -inv return the 6-digit .inv file name for precomputed
				// inverted index cached for all .xml records in leaf folder
				if eutils.IsAllDigits(file) {
				...
				if eutils.IsAllDigitsOrPeriod(file) {
	...
	if ftch != "" && indx == "" {
		...
		strq := eutils.CreateFetchers(ftch, sfx, zipp, uidq)
	...
	if strm != "" && indx == "" {
		...
		uidq := eutils.CreateUIDReader(in)
		strq := eutils.CreateCacheStreamers(strm, sfx, uidq)
		unsq := eutils.CreateXMLUnshuffler(strq)
	...
	if stsh != "" && indx != "" {
		...
		stsq := eutils.CreateStashers(stsh, parent, indx, sfx, hshv, zipp, asn, 1000, xmlq)
